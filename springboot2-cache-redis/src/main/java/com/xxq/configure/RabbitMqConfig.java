...
import com.xxq.domain.mq.callback.MsgSendConfirmCallBack;
...
public class RabbitMqConfig {
    ...
    public Binding bindingOne() {
        Queue queue = queueConfig.firstQueue();
        DirectExchange exchange = exchangeConfig.directExchange();
        return BindingBuilder.bind(queue).to(exchange).with(RabbitMqConfig.ROUTINGKEY1);
    }
    /**
     * 将消息队列2和交换机进行绑定
     */
    ...
    public Binding bindingTwo() {
        Queue queue = queueConfig.secondQueue();
        DirectExchange exchange = exchangeConfig.directExchange();
        return BindingBuilder.bind(queue).to(exchange).with(RabbitMqConfig.ROUTINGKEY2);
    }
    /**
