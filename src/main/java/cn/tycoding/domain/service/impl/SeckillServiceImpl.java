package cn.tycoding.domain.service.impl;
...
import cn.tycoding.infrastructure.persistence.entity.SeckillOrder;
import cn.tycoding.common.enums.SeckillStatEnum;
...
import cn.tycoding.infrastructure.persistence.mapper.SeckillMapper;
import cn.tycoding.infrastructure.persistence.mapper.SeckillOrderMapper;
import cn.tycoding.domain.service.SeckillService;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    @Override
    public ExposerResp exportSeckillUrl(long seckillId) {
        ...
        if (seckill == null) {
            ...
            if (seckill == null) {
                //说明没有查询到
                return new ExposerResp(false, seckillId);
            } else {
        ...
        if (nowTime.getTime() < startTime.getTime() || nowTime.getTime() > endTime.getTime()) {
            return new ExposerResp(false, seckillId, nowTime.getTime(), startTime.getTime(), endTime.getTime());
        }
        //转换特定字符串的过程，不可逆的算法
        String md5 = getMD5(seckillId);
        return new ExposerResp(true, md5, seckillId);
    }
    ...
    @Override
    @Transactional
    public SeckillExecutionResp executeSeckill(long seckillId, BigDecimal money, long userPhone, String md5)
        ...
        try {
            ...
            if (insertCount <= 0) {
                ...
                if (updateCount <= 0) {
                    ...
                    return new SeckillExecutionResp(seckillId, SeckillStatEnum.SUCCESS, seckillOrder);
                }
            }
